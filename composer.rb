...
class ComposerBuild
  ...
  def save
    if @C_id == nil
      result = @conn.exec('INSERT INTO classical_composers (first_name, ' \
      ...
      @conn.exec('UPDATE classical_composers SET ' \
  ...
  def connect_to_pieces(input)
   result = @conn.exec(" SELECT * FROM classical_composers WHERE last_name == #{composer_last_name} RETURNING id")
   return id = result[0]['id'].to_i
  end
end

def create_table(conn)
  conn = PG.connect(dbname: 'test')
  conn.exec('CREATE TABLE IF NOT EXISTS classical_composers' \
