...
def create_table(conn)
  conn = PG.connect(dbname: 'test')
  conn.exec('CREATE TABLE IF NOT EXISTS two_pieces(id SERIAL PRIMARY KEY, composer_last_name VARCHAR, date_composed INTEGER, tonic VARCHAR, period VARCHAR, orchestration VARCHAR)')
end

def alter_foreign(conn)
  "ALTER TABLE two_pieces
  ADD FOREIGN KEY (composer_id)
  REFERENCES two_composers(composer_id)"
end
